...
const login = asyncHandler(async(req,res)=>{
    ...
    if(findUser && await findUser.isCorrectPassword(password)){
        ...
        await User.findByIdAndUpdate(findUser._id,{newRefreshToken},{ new: true });
...
const deleteUser = asyncHandler(async(req,res)=>{
    ...
    return res.status(200).json({
        success: response ? true: false,
        message: response ? `User with email: "${response.email}" has deleted`: "Can not found user.No user delete",
    });
});

const updateUser = asyncHandler(async(req,res)=>{
    const { _id } = req.user;
    if(!_id || Object.keys(req.body).length === 0) throw new Error(`Missing params`);
    const response = await User.findByIdAndUpdate(_id,req.body,{new:true}).select("-password -role");
    return res.status(200).json({
        success: response ? true: false,
        updateUser: response ? "Updated successfully": "User has not been updated.",
    });
});

const updateUserByAdmin = asyncHandler(async(req,res)=>{
    const { uid } = req.query;
    if(Object.keys(req.body).length === 0) throw new Error(`Missing params`);
